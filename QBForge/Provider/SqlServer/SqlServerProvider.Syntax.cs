...
namespace QBForge.Provider
{
	partial class SqlServerProvider
	{
		public override string AppendObject(string objectName, ReadabilityLevels level = ReadabilityLevels.Default)
		{
			...
			{
				if (ContainsOnlyLatinOrNumbersOrUnderscore(objectName) && !IsQuotedKeyword(objectName))
				{
					return objectName;
				}
			}
		...
		public override string AppendLabel(string label, ReadabilityLevels level = ReadabilityLevels.Default)
		{
			...
			{
				if (ContainsOnlyLatinOrNumbersOrUnderscore(label) && !IsQuotedKeyword(label))
				{
					return label;
				}
			}
		...
		private static bool ContainsOnlyLatinOrNumbersOrUnderscore(string value)
		{
